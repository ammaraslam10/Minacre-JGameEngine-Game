...
//Update CollisionMaskList, getx gety debug, collisionboxtest, collisionpointtest
//Update CameraFollowing(obj, double, double), windowIcon(), textCreateFont() textFont(), ~textSize()
/*
...
public class JGameEngine {
	...
	if(window != null) {
	    window.gameLoop_can_run = false;
	    while(window.gameLoop_running) {}
	}	
	...
	private volatile boolean running; volatile boolean gameLoop_can_run = true; volatile boolean gameLoop_running;
	...
	private void addFont(String name, Font font) {
		...
		if(this.name.get(i).equals(name)) return;
	    this.name.add(name);
	    this.font.add(font);
	}
	private Font getFont(String name) {
	    for(int i = 0; i < this.name.size(); i++)
		if(this.name.get(i).equals(name)) return this.font.get(i);	
	    return null;
	}
    }
    //~~~~~~~~~~ Fonts Ends
	...
	//window.g.setFont(window.g.getFont().deriveFont(window.g.getFont().getSize()));
    ...
    public void textFontSystem(String name, String type, int size) { int t = Font.PLAIN; if(type.equals("bold")) t = Font.BOLD; else if(type.equals("italic")) t = Font.ITALIC; draw().setFont(new Font(name, t, size * (int) camera.d)); }
    /** Change the font of text to a custom font from a given file, ttf format supported only. */  
    public Font textCreateFont(String path, float size) { 
	Font font = fonts.getFont(path+"~"+size); if(font != null) return font;
    ...
    public void textFont(Font font) { draw().setFont(font); }
    public void textFont(String path, float size) { draw().setFont(textCreateFont(path, size)); }
	...
	/// Draw the collision mask to debug
	public void debug() {
    ...
    ArrayList<CollisionMask> collisionMaskList(Object obj) {
    ...
    ArrayList<CollisionMask> collisionPointTest(double x, double y) {
	return collisionBoxTest(x,y,1,1);
    }    
    /** Get the list of objects that are collide to the given box */
    ArrayList<CollisionMask> collisionBoxTest(double x, double y, int w, int h) {
